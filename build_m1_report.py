...
SUBTITLE = "Milestone M1 — Identity & Family Profiles"
...
SECTIONS = [
    ("1. M1 完成总结", [
        ...
        ("bullet", "自动化验收：npm run validate 共 83 项检查全部通过（M0 基线 67 + M1 新增 16）。其中「跨用户隔离（9–11）」「共享运行时打包（15a–d）」「默认档案持久化（13）」三项关键验收均通过。"),
        ("p", "结论：依据验收门槛（owner-isolation、shared-runtime packaging、default-profile persistence 必须全部通过），M1 可判定为【完成】。"),
...
"""),
    ...
    ("11. 自动化校验结果", [
        ("p", "npm run validate 串联 typecheck 与 83 项检查：M0 基线 67 + M1 新增 16，0 失败。M1 十六项分组如下："),
        ("table", ["编号", "检查", "分组"], [
        ...
        ("p", "结论：跨用户隔离（9–11）、共享运行时打包（15a–d）、默认档案持久化（13）三项关键验收全部通过，故 M1 可判定完成。"),
    ]),
    ("12. 手动测试清单", [
        ("p", "详见 docs/MANUAL_TEST_CHECKLIST.md。要点："),
        ("bullet", "A. DevTools：首次运行进入引导；创建一个档案后自动默认；再建第二个；切换默认；编辑；选择活动档案；重新编译后默认仍生效；提交空姓名/非法关系被拒；连续点击提交不重复创建。"),
        ("bullet", "B. 真机：上述流程在手机端复现；离线时首页横幅正确；本地 ID 失效回退正确。"),
        ("p", "注：单设备上难以实测「跨账户隔离」，该路径由自动化测试（9–11）覆盖，真机仅做冒烟。"),
    ]),
    ("13. 文档更新与决策记录", [
        ("h2", "13.1 新增文档"),
    ...
    ("14. Git 与范围控制", [
        ("p", "分支：feature/m1-identity-family-profiles，自干净的 M0 基线（cc698c2）切出。"),
        ("p", "聚焦提交（5 个）："),
        ("bullet", "M1: shared runtime (repository, user/profile services, session) + build packaging"),
        ("bullet", "M1: profileApi cloud function + login uses shared runtime; drop openid from client responses"),
        ("bullet", "M1: client identity/profile UI + auth/profile/session services"),
        ("bullet", "M1: documentation (...) + ignore .workbuddy"),
        ("bullet", "M1: extend validation with 16 M1 acceptance tests"),
        ("p", "范围控制：未包含任何 M2+ 代码。附带修正：M0 的 cloudfunctions/**/*.js 误忽略导致云函数源码从未入库；M1 修正 .gitignore 改为仅忽略 lib/shared/，使 login/mealApi/aiAnalyze 源码得以纳入版本控制。"),
    ]),
    ("15. M2 建议（不实现）", [
        ("p", "建议 M2 聚焦「用餐记录与营养」："),
...
def build_docx(path):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("状态：✅ 完成（83/83 校验通过）")
...
def build_md(path):
    ...
    out.append(f"> 日期：{REPORT_DATE} ｜ 状态：✅ 完成（83/83 校验通过）\n")
    out.append("\n## 目录\n")
    for idx, (title, _) in enumerate(SECTIONS, start=1):
