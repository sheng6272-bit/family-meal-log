...
"""),
    ...
    ("11. 自动化校验结果", [
        ("p", "npm run validate 串联 typecheck 与 91 项检查：M0 基线 67 + M1/M1.1 共 24，0 失败。已执行（本次 M1.1 提交前）实测通过。分组如下（14a–15f 为 M1.1 新增/调整）："),
        ("table", ["编号", "检查", "分组"], [
        ...
        ("p", "结论：跨用户隔离（9–11）、请求级幂等（14a–c）、默认档案单一事实来源（15a–f）、共享运行时打包（16a–d）全部通过，M1 + M1.1 关键验收达成。"),
    ]),
    ("12. 手动测试清单", [
